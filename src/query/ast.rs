...
impl Expr {
    pub fn caller(&self, sample_idx: &HashMap<String, usize>, heterozygosity: Prob) -> Box<call::Caller> {
        match self {
            &Expr::SampleUnion(ref samples) => Box::new(call::SampleUnion {
                samples: samples.iter().map(|sample| *sample_idx.get(sample).expect("Unknown sample name.")).collect(),
                heterozygosity: heterozygosity,
                ploidy: 2, // TODO extend to arbitrary ploidy once samtools mpileup has catched up
            }),
